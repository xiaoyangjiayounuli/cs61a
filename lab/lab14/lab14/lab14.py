...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    #image slipting every element in s into two group, so for every element, there are two choices.
    # so => return helper(s, different + now_one) + helper(s, different - now_one) until the original s become empty
    # base case:the original s become empty then if the different <= d, the splits counts
    def helper(s, different):
        if not s:
            if abs(different) <= d:
                return 1
            return 0
        now_one = s[0]
        s = s[1:]
        return helper(s, different + now_one) + helper(s, different - now_one)
    return helper(s, 0)//2
