...
# BEGIN Problem 6
class WallAnt(Ant):
    #We are going to add some protection to our glorious home base by implementing the WallAnt, which is an ant that does nothing each turn
    name='Wall'
    food_cost=4
    implemented=True
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.chewing==0 and self.place.bees[:]:
            select_bees=bee_selector(self.place.bees)
            select_bees.health=0
            select_bees.reduce_health(0)
...
# END Problem EC
    ...
    # BEGIN Problem EC
    implemented = True
    is_truequeen = True   # Change to True to view in the GUI
    # END Problem EC
