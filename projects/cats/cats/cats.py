...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    differ_list=[diff_function(typed_word,word,limit) for word in valid_words]
    if min(differ_list)>limit:
        return typed_word
    else:
        return valid_words[differ_list.index(min(differ_list))]
...
def time_per_word(times_per_player, words):
    ...
    for tpp in times_per_player:
        diff.append([tpp[i]-tpp[i-1] for i in range(1,len(tpp))])#len(tpp）再这里是[6，3，6，2]的总长度，所以可以用
...
def fastest_words(game):
    ...
    for word_indice in word_indices:
        ...
        for player_indice in player_indices:
            lst2.append(time(game,player_indice,word_indice))
        lst.append(lst2)
    lst3=[]
    for i in lst:
        lst3.append(i.index(min(i)))
    lst4=[]
    for c in player_indices:
        lst4.append([])
    for d in range(len(lst3)):
        playr_id=lst3[d]
        lst4[playr_id]=lst4[playr_id]+[word_at(game,d)]
    return lst4
